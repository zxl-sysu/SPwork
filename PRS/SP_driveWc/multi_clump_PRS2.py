...
if __name__=='__main__':
    ...
    filter_dict['SP2_SM.d18.2.22.0']=r"./sig/SP2_SM.d18.2.22.0.txt"

    for filter_name,filter_path in filter_dict.items():
        df=pd.read_csv(filter_path,sep='\t')
        df=df.rename({'CHR':'chr.exposure',
            'POS':'pos.exposure',
        ...
        #print(filter_name,df.head())
        df['pval.exposure']=df['pval.exposure'].astype('float')
        df['chr.exposure']=df['chr.exposure'].astype('int')
        df['pos.exposure']=df['pos.exposure'].astype('int')

        df['pval.exposure']=df['pval.exposure'].astype('float')
        df['eaf.exposure']=df['eaf.exposure'].astype('float')

        df=df[df['pval.exposure']<1E-5]
        df=df[(df['eaf.exposure']<0.99) & (df['eaf.exposure']>0.01)]

        df.sort_values(by=['pval.exposure'],inplace=True)
        print(filter_name,df.head(),df.shape)
        filter_dict[filter_name]=df
    filter_snps=[j['SNP.exposure'].tolist() for i,j in filter_dict.items()]
    #print([j for i in filter_snps for j in i])
    filter_snps=[j for i in filter_snps for j in i]
    print(filter_snps)
    ...
    PRS_dir='.'
    disease_name='WC' ##改成表型的名称，用于命名结果文件
    os.chdir(PRS_dir)
    print('PRS dir = %s' % PRS_dir)
    print('changing working dir to %s'% os.getcwd() )

    pt=[0.99] #若有希望运行的其它P阈值，可修改 5e-1,5e-2,5e-3,5e-4,5e-5,5e-6,5e-7,5e-8
    pt.sort()
    print(pt)
    clump_kb=1000
    ...
    for clump_r2 in clump_r2_list:
        ...
        score=pd.read_csv(r'./WC/qc_base.txt',sep='\s+')
        score=score[score['rsID'].isin(filter_snps)]
        score.to_csv('qcFilter.txt',index=None,sep='\t')
        ...
        for p in pt:
            ...
            if os.path.exists(clump_path):
                ...
                continue
            cho_clp_snp=pd.read_csv(clump_path,sep='\s+')
            ...
            if score_files:
                for file in score_files:
                    ...
                    prs_result=pd.read_csv('%s/%s_prs_chr%s.sscore'%(save_dir,trait,i) ,sep='\s+')
